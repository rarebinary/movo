...
- Gate 0 recovery is now evidenced on both the disposable account and an explicitly authorized current-account no-op: store bytes and stable metadata survive atomically, the visible lab wallpaper remained unchanged, and the only observed exact-xattr delta is macOS's protected volatile `com.apple.provenance` marker.
- Gate 2 is closed: the arm64 `com.apple.wallpaper` extension builds, embeds,
  preserves sandbox entitlements under ad-hoc signing, and completes clean
  install/status/disable/enable/repair/remove/reinstall cycles from
  `~/Applications` without changing wallpaper selection.
- Phases 4 and 7–9 remain active. In particular, **Set Wallpaper** still reports the honest unsupported-integration notice until the renderer lifecycle and transactional rollback path are proven on this macOS build.
